...
#define SU32(_u_)                                                                                                      \
    (tb64lutse[(_u_ >> 8) & 0x3f] << 24 | tb64lutse[(_u_ >> 14) & 0x3f] << 16 | tb64lutse[(_u_ >> 20) & 0x3f] << 8     \
...
#define ETAIL()                                                                                                        \
    ...
    if (_l == 3)                                                                                                       \
    {                                                                                                                  \
        unsigned _u = ip[0] << 24 | ip[1] << 16 | ip[2] << 8;                                                          \
        stou32(op, SU32(_u));                                                                                          \
        op += 4;                                                                                                       \
        ip += 3;                                                                                                       \
    }                                                                                                                  \
    ...
    {                                                                                                                  \
        *op++ = tb64lutse[(ip[0] >> 2) & 0x3f];                                                                        \
        if (_l == 2)                                                                                                   \
            *op++ = tb64lutse[(ip[0] & 0x3) << 4 | (ip[1] & 0xf0) >> 4], *op++ = tb64lutse[(ip[1] & 0xf) << 2];        \
        else                                                                                                           \
            *op++ = tb64lutse[(ip[0] & 0x3) << 4], *op++ = '=';                                                        \
        *op++ = '=';                                                                                                   \
    }

extern const unsigned short tb64lutxe[];
#define XU32(_u_) (tb64lutxe[(_u_ >> 8) & 0xfff] << 16 | tb64lutxe[_u_ >> 20])

#define EXTAIL()                                                                                                       \
    for (; op < (out + outlen) - 4; op += 4, ip += 3)                                                                  \
    {                                                                                                                  \
        unsigned _u = BSWAP32(ctou32(ip));                                                                             \
        stou32(op, XU32(_u));                                                                                          \
    }                                                                                                                  \
    ETAIL()
//--------------------- Decoding ----------------------------------------------------------
...
#define DU32(_u_)                                                                                                      \
    (tb64lutxd0[(unsigned char)(_u_)] | tb64lutxd1[(unsigned char)(_u_ >> 8)] | tb64lutxd2[(unsigned char)(_u_ >> 16)] \
     | tb64lutxd3[_u_ >> 24])
...
static ALWAYS_INLINE size_t _tb64xd(const unsigned char* in, size_t inlen, unsigned char* out)
{
    ...
    unsigned u = 0, l = (uint32_t)((in + inlen) - ip);
    if (l == 4) // last 4 bytes
        if (ip[3] == '=')
        {
            l = 3;
...
#    define MM_PACK8TO6(v, cpv)                                                                                        \
        {                                                                                                              \
            const __m128i merge_ab_and_bc = _mm_maddubs_epi16(                                                         \
                v, _mm_set1_epi32(0x01400140)); /*/dec_reshuffle: https://arxiv.org/abs/1704.00605 P.17*/              \
            v = _mm_madd_epi16(merge_ab_and_bc, _mm_set1_epi32(0x00011000));                                           \
            v = _mm_shuffle_epi8(v, cpv);                                                                              \
        }

#    define MM_MAP8TO6(iv, shifted, delta_asso, delta_values, ov)                                                      \
        { /*map 8-bits ascii to 6-bits bin*/                                                                           \
