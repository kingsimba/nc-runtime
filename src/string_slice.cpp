...
bool StringSubsliceIter::next(StringSlice* cOut, Range* rangeOut) {
  ...
  *cOut = StringSlice(m_str, (int)(newStr - m_str), retain(m_ncstring));
  if (rangeOut != NULL) *rangeOut = Range((int)(m_str - m_strBegin), cOut->length());
...
StringSlice StringSlice::makeWithString(NcString* str) { return StringSlice(str->cstr(), str->length(), retain(str)); }
...
StringSlice::~StringSlice() { release((NcObject*)m_ncstring); }
...
bool StringTokenIter::next(StringSlice* sliceOut, Range* rangeOut) {
  ...
  *sliceOut = StringSlice(firstNonSep, (int)(m_str - firstNonSep), retain(m_ncstring));
  if (rangeOut != NULL) *rangeOut = Range((int)(firstNonSep - m_strBegin), sliceOut->length());
